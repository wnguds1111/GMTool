...
for i in range(10):
    ...
    if applied:
        ...
        # Button on the left of 0/0
        shard_html = f"""<div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                            <button class="btn btn-outline" style="padding: 2px 10px; font-size: 13px;">샤드 적용</button>
                            <span>{shard_text}</span>
                        </div>"""
    ...
    # Center align Title by removing text-align: left and padding
    rows_all += f"""
                <tr>
...
print("Updated mail_all.html table with title centered and button left of 0/0.")
